...
```
Note that the best model here was a __ETS(AAM)__ which means we are fitting an additive random error, an additive trend and a multiplicative seasonal component.
...
**fixed** optional numeric vector with values for the components variances in the following order $fixed=c(\sigma^2_\eta,\sigma^2_\xi,\sigma^2_\omega,\sigma^2_\epsilon)$. 
__(level, trend, seasonal, observation)__ The first variance is with respect to level, second is trend, third is seasonal and fourth is observation. If supplied, only NA entries in fixed will be varied. This argument is useful for setting variances to zero.

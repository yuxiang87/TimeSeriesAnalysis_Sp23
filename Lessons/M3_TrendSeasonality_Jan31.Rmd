...
ts_inflow_data_t <- ts(inflow_data[,2:(2+nhydro-1)], start = c(2001,1), end = 2002, frequency =12) ; 
...
for(i in 1:nhydro){
  par(mfrow=c(1,2))
  acf(ts_inflow_data[,i], lag.max = 40, main= paste0("HP",i))
...
lm(ts_inflow_data[, iHP] ~ t)


  par(mfrow=c(1,2))
  acf(ts_inflow_data[,4], lag.max = 40, main= paste0("HP",i))
  pacf(ts_inflow_data[,4], lag.max = 40, main= paste0("HP",i))


plot(acf(ts_inflow_data[,4]))
plot(t,ts_inflow_data[, iHP],type = "l" )
##correct one:

...
```{r Q1}
#Q1
df_biomP_renewP_hydroC <- data.frame("BiomP"=A02_rawdata$`Total Biomass Energy Production`, "RenewP"=A02_rawdata$`Total Renewable Energy Production`, "HydroC"=A02_rawdata$`Hydroelectric Power Consumption`)
...
plot_renewP <- ggplot(df_date_brh, aes(x=Time, y=RenewP)) +
  ...
  ggtitle("Total renewable energy production")+ xlab("Year")+ylab("Energy Production (Trillion Btu)")
plot_renewP
...
plot_hydroC <- ggplot(df_date_brh, aes(x=Time, y=HydroC)) +
  ...
  ggtitle("Hydroelectric power consumption")+ xlab("Year")+ylab("Energy Consumption (Trillion Btu)")
plot_hydroC
